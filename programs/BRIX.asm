...
Draw_Brick_Row:
    ...
    SE  VB, 18     ; If all rows drawn, continue on
...
Wait:
    ...
    RND V6, 15 ; Get random coord for ball start
    LD  V7, 30  ; Set Y coord of ball start
    LD  V8, 1   ; Set X direction to RIGHT
    LD  V9, 255 ; Set Y direction to UP
...
Loop:
    ...
    LD  V0, #3F  ; Set highest X coord.
    AND V6, V0  ; AND ball X pos. with V0

    LD  V1, #1f  ; Set highest Y coord
    AND V7, V1  ; AND ball Y pos. with V1

    SNE V7, #1f  ; If ball not at bottom, skip
...
Check_Paddle:
    ...
    LD  V8, 255 ; Set X direction to LEFT
...
Bottom:
    LD  V9, 255 ; Ball is at bottom, so set direction to UP
...
Paddle_Beep:
    ...
    LD  V9, 255      ; Set ball direction to UP
